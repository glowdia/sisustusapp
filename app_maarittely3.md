# Sisustusapp - vaatimusmäärittely ja toteutuskuvaus (versio 3)
...
**Versio 3 -muutokset:** Tähän versioon on lisätty tarkennukset, jotka tekevät dokumentista paremman toteutuksen ohjausdokumentin: nykytila vs. tavoitetila (luku 6.1), kevyt organisaatiopohja myöhempää kaupallistamista varten (luku 11), konkreettinen `canvas_json v1` -rakenne (luku 11.1), tuotepankin muokkaus- ja poisto-oikeudet (luku 9.5), tyylitestilinkin token-kentät tietomalliin (luku 11), pohjakuvan PDF-käsittelypäätös (luku 8.7), API-arkkitehtuurin linjaus (luku 13.3) sekä täsmennys siitä, mitkä työkalut ovat nykytilaa ja mitkä rakennetaan vaiheessa 0.
...
- **Suunnittelija**
  ...
  - käyttää organisaation yhteistä tuotepankkia
  - lisää manuaalisia tuotteita organisaation yhteiseen tuotepankkiin
...
Repositorion juureen luodaan `AGENTS.md`, joka sisältää Codexille pysyvät ohjeet:
...
### 6.1 Nykytila vs. tavoitetila (uusi versiossa 3)

Tämä dokumentti kuvaa sekä nykyisen prototyypin että tavoitetilan. Kaikki dokumentissa mainitut työkalut ja rakenteet eivät ole vielä käytössä nykyisessä repositoriossa.

**Nykytila repositoriossa:**

- Next.js-sovellus toimii paikallisesti.
- Editorin perusrakenne on olemassa.
- Tuotepankki luetaan CSV-tiedostosta.
- Tikkurila-väridata luetaan CSV-tiedostosta.
- Tallennus tapahtuu prototyypissä selaimen localStorageen.
- `package.json` sisältää tällä hetkellä komennot:
  - `npm run dev`
  - `npm run build`
  - `npm run lint`
  - `npm run typecheck`

**Tavoitetila sisäiseen MVP:hen:**

- Supabase Auth, Postgres, Storage ja RLS ovat käytössä.
- Moodboard-data tallentuu Supabaseen.
- Tuotekuvat ja käyttäjän uploadit tallentuvat hallitusti Storageen tai kulkevat turvallisen proxy-ratkaisun kautta.
- Vitest, Playwright, Zod, Sentry ja GitHub Actions on lisätty projektiin.
- Tietokantaskeema on versionhallinnassa Supabase-migraatioina.
- Projektilla on `AGENTS.md` sekä `TYOLOKI.md`.

**Tärkeä toteutusperiaate:** vaiheessa 0 rakennetaan puuttuvat kehityksen suojarakenteet ennen kuin sovellukseen lisätään merkittäviä uusia ominaisuuksia.
...
**Päätös PDF-pohjakuvasta (uusi versiossa 3):**

- Jos käyttäjä lataa pohjakuvan PDF:nä, MVP näyttää moodboardissa PDF:n ensimmäisen sivun kuvana.
- PDF muunnetaan esikatseltavaksi kuvaksi joko selainpuolella tai palvelinpuolella valitulla kirjastolla.
- Alkuperäinen PDF säilytetään Supabase Storagessa.
- Moodboardiin tallennetaan sekä alkuperäisen PDF:n tiedostopolku että renderöidyn esikatselukuvan tiedostopolku.
- Monisivuisen PDF:n muut sivut jätetään MVP:ssä huomioimatta, ellei käyttäjä erikseen valitse myöhemmässä vaiheessa sivua.
- Jos PDF-renderöinti epäonnistuu, käyttäjälle näytetään selkeä virhe ja pyydetään lataamaan pohjakuva kuvamuodossa.
...
- Manuaalisesti lisätty tuote lisätään heti organisaation yhteiseen tuotepankkiin.
...
### 9.5 Tuotepankin muokkaus- ja poisto-oikeudet (uusi versiossa 3)

Koska tuotepankki on yhteinen kaikille suunnittelijoille, tuotteiden muokkaus ja poistaminen määritellään erikseen.

**MVP:n oikeusmalli:**

- Kaikki organisaation kirjautuneet suunnittelijat voivat nähdä organisaation aktiiviset tuotteet.
- Kaikki organisaation kirjautuneet suunnittelijat voivat lisätä uuden manuaalisen tuotteen organisaation yhteiseen tuotepankkiin.
- Manuaalisen tuotteen lisännyt käyttäjä voi muokata itse lisäämäänsä tuotetta.
- Tuotetta ei poisteta fyysisesti tietokannasta, vaan se deaktivoidaan `is_active = false`.
- Tuotteen lisännyt käyttäjä voi deaktivoida itse lisäämänsä tuotteen.
- CSV- tai feed-tuotteita voi MVP:ssä muokata vain admin tai erillinen ylläpitoprosessi.
- Jos tuotetta on jo käytetty moodboardissa, vanha moodboard säilyttää oman snapshotinsa tuotteen tiedoista, vaikka tuotepankin tuote myöhemmin muuttuu.

**Tärkeä dataperiaate:** kun tuote lisätään moodboardille, moodboard-elementtiin tallennetaan tuotteen senhetkiset esitystiedot snapshotina. Elementti voi silti sisältää myös `product_id`-viitteen alkuperäiseen tuotteeseen. Näin vanhat asiakassuunnitelmat eivät muutu yllättäen, jos tuotepankin tuotetta päivitetään myöhemmin.
...
Tietomalli rakennetaan sisäisen MVP:n tarpeisiin, mutta siihen lisätään kevyt organisaatiopohja. Tämä ei tarkoita täyttä SaaS-tuotetta vielä, mutta se ehkäisee suurimman uudelleenrakennuksen, jos palvelu päätetään myöhemmin tuotteistaa.

MVP:ssä järjestelmässä voi olla vain yksi organisaatio, esimerkiksi oma yritys. Silti keskeisiin tauluihin lisätään `organization_id`, jotta data voidaan myöhemmin erottaa asiakasyrityksittäin.

### `organizations` (uusi versiossa 3)

Yritys tai tiimi, jonka alla käyttäjät, projektit ja tuotteet ovat.
...
### `organization_members` (uusi versiossa 3)

Käyttäjän jäsenyys organisaatiossa.

- `id` UUID
- `organization_id` UUID, viite `organizations.id`
- `user_id` UUID, viite `profiles.id`
- `role` text, esim. `designer`, `admin`, `owner`
- `created_at` timestamp

MVP:ssä käyttäjä kuuluu yhteen organisaatioon. Myöhemmässä kaupallisessa versiossa sama käyttäjä voi tarvittaessa kuulua useampaan organisaatioon.
...
- `email` text
- `full_name` text
- `default_organization_id` UUID, viite `organizations.id`
- `created_at` timestamp
- `updated_at` timestamp
...
### 11.1 `canvas_json v1` -rakenne (uusi versiossa 3)

`canvas_json` tallentaa yhden moodboard-sivun editoridatan. Rakenne pidetään mahdollisimman yksinkertaisena, mutta kaikki elementit sisältävät yhteiset peruskentät.

**Sivun canvas-rakenne:**
...
**Kaikkien elementtien yhteiset kentät:**
...
```json
{
  ...
  "snapshot": {
    ...
    "priceText": "599,00 €",
    "dimensionsText": "Leveys 197 cm; syvyys 92 cm; korkeus 82 cm",
...
```json
{
  ...
  "locked": false,
  "zIndex": 1,
  "storagePath": "project-uploads/user/project/image.png",
  "sourceUrl": null,
  "altText": "",
  "fit": "contain"
}
...
```json
{
  ...
  "locked": false,
  "zIndex": 1,
  "originalStoragePath": "project-uploads/user/project/floorplan.pdf",
  "previewStoragePath": "project-uploads/user/project/floorplan-preview.png",
  "fileType": "pdf"
}
```

**Pakolliset toteutusohjeet:**

- `schemaVersion` on aina mukana sekä `canvas_json`-juuressa että `moodboard_pages.schema_version`-kentässä.
- Uusia elementtityyppejä ei lisätä ilman skeeman, migraation ja testien päivitystä.
- Tuote-elementti käyttää snapshotia, jotta vanhat moodboardit eivät muutu, vaikka tuotepankin tuote muuttuu.
- Kaikki mitat ovat canvas-koordinaatiston pikseleitä, eivät CSS-pikseleitä.
- PDF-export käyttää samaa dataa kuin editori, mutta jättää editorin hallintaelementit pois.

### `products`

Organisaation yhteinen tuotepankki.
...
- `customer_name` text
- `customer_email` text
- `token_hash` text, julkisen täyttölinkin token tallennettuna hajautettuna
...
- organisaation yhteisen tuotepankin kuvat voidaan tehdä sisäisesti organisaation kirjautuneiden käyttäjien luettaviksi
...
- organisaatiokohtaisissa tauluissa RLS rajaa datan käyttäjän `organization_members`-jäsenyyden perusteella
- organisaation yhteinen tuotepankki on kyseisen organisaation kirjautuneiden suunnittelijoiden luettavissa
...
### 13.3 API-arkkitehtuurin linjaus (uusi versiossa 3)

Sovelluksessa käytetään kahta erilaista tiedonkulkumallia. Tämä määritellään etukäteen, jotta AI ei toteuta samaa asiaa monella eri tavalla.

**Client + Supabase RLS:**

Näitä tietoja voidaan lukea ja kirjoittaa suoraan selaimesta Supabase clientilla, kun RLS-politiikat on määritelty oikein:

- kirjautuneen käyttäjän omat projektit
- projektin moodboardit
- moodboardin sivut ja `canvas_json`
- käyttäjän omat upload-metatiedot
- tuotepankin lukeminen
- Tikkurila-värien lukeminen

**Server/API-reitit:**

Näissä toiminnoissa käytetään aina Next.js API routea tai server actionia:

- OpenAI-kutsut
- PDF-export, jos selainpohjainen export ei riitä
- image proxy ulkoisille tuotekuville
- CSV-importit
- feed/API-integraatiot
- manuaalisen tuotteen kuvan käsittely, jos kuva haetaan URL-osoitteesta
- token-pohjainen julkinen tyylitestin lähetys
- kaikki admin-toiminnot

**Periaate:** selaimesta ei koskaan kutsuta palveluita, jotka vaativat salaisia avaimia. Supabasen anon-avain saa olla selaimessa, mutta service role -avainta, OpenAI-avainta tai muita salaisuuksia ei koskaan tuoda client-koodiin.
...
- token näytetään käyttäjälle linkissä vain kerran ja tietokantaan tallennetaan vain tokenin hajautettu arvo (`token_hash`)
...
- luodaan `AGENTS.md`-ohjetiedosto (ks. luku 4.1)
- luodaan `TYOLOKI.md`-pohja
- lisätään puuttuvat kehitystyökalut projektin riippuvuuksiin: Vitest, Playwright, Zod, Supabase client/SSR-kirjastot ja Sentry
- lisätään `package.json`-skriptit vähintään: `test`, `test:e2e`, `test:unit`, `ci`
...
- tallennetaan manuaaliset tuotteet organisaation yhteiseen tuotepankkiin
...
- projektissa on selkeä jatkokehitysdokumentaatio (`AGENTS.md`, `TYOLOKI.md`)
